...
import {
  ...
  AlertTriangle, TrendingDown, Calendar, Target, Clock, LogOut,
} from "lucide-react";
import {
...
import { PiggyBankGraphic } from "@/components/PiggyBank";
...
function ExpensesPage({ expenses, setExpenses, triggerOpen }: any) {
  ...
  const [name, setName] = useState(""); const [value, setValue] = useState(0);
  const [day, setDay] = useState(""); const [month, setMonth] = useState(String(currentMonth));
  const [category, setCategory] = useState("moradia"); const [essential, setEssential] = useState(false); const [fixed, setFixed] = useState(false);

  useEffect(() => { if (triggerOpen > 0) openNew(); }, [triggerOpen]);

  const openNew = () => { setEditId(null); setName(""); setValue(0); setDay(""); setMonth(String(currentMonth)); setCategory("moradia"); setEssential(false); setFixed(false); setModal(true); };
  const openEdit = (e: any) => { setEditId(e.id); setName(e.name); setValue(e.value); setDay(String(e.dueDay || "")); setMonth(String(e.dueMonth || currentMonth)); setCategory(e.category); setEssential(e.essential); setFixed(!!e.fixed); setModal(true); };
  const save = () => {
    if (!name || !value) return;
    const obj = { name, value, dueDay: parseInt(day) || null, dueMonth: parseInt(month) || currentMonth, category, essential, fixed };
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 4, marginTop: 3 }}>
              ...
              {e.fixed && <Tag color={C.purple}>Fixa</Tag>}
            </div>
          </div>
    ...
    <Modal open={modal} onClose={() => setModal(false)} title={editId ? "Editar Despesa" : "Nova Despesa"}>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          ...
          <Checkbox checked={fixed} onChange={() => setFixed(!fixed)} label="Fixa (recorrente)" color={C.purple} />
        </div>
        <Btn primary onClick={save} style={{ width: "100%", justifyContent: "center", marginTop: 4 }}>{editId ? "Salvar" : "Adicionar"}</Btn>
      </div>
    </Modal>
...
// ══════════════════════════════════════════
function ReservePage({ essentialMonthly, reserve, setReserve }: { essentialMonthly: number; reserve: { targetMonths: number; currentAmount: number }; setReserve: (r: { targetMonths: number; currentAmount: number }) => void }) {
  const [amountInput, setAmountInput] = useState(reserve.currentAmount);
  const target = essentialMonthly * reserve.targetMonths;
  const percentage = target > 0 ? Math.min(100, (reserve.currentAmount / target) * 100) : 0;

  return <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 560 }}>
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
        <div>
          <p style={{ fontSize: 15, fontWeight: 600, color: C.text, margin: 0 }}>Reserva de emergência</p>
          <p style={{ fontSize: 12, color: C.dim, margin: "4px 0 0" }}>
            Meta: {reserve.targetMonths}x suas despesas essenciais ({fmt(essentialMonthly)}/mês) = {fmt(target)}
          </p>
        </div>
      </div>

      <div style={{ display: "flex", justifyContent: "center", padding: "18px 0" }}>
        <PiggyBankGraphic percentage={percentage} />
      </div>

      <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
        <div style={{ flex: 1 }}>
          <Label>Valor guardado hoje</Label>
          <CurrencyInput value={amountInput} onChange={setAmountInput} />
        </div>
        <div style={{ width: 120 }}>
          <Label>Meses de meta</Label>
          <TextInput value={String(reserve.targetMonths)} onChange={(v: string) => setReserve({ ...reserve, targetMonths: parseInt(v) || 1 })} />
        </div>
      </div>
      <Btn primary onClick={() => setReserve({ ...reserve, currentAmount: amountInput })} style={{ width: "100%", justifyContent: "center" }}>
        Salvar
      </Btn>
    </Card>
    ...
    {essentialMonthly === 0 && (
      <Card>
        <p style={{ fontSize: 12, color: C.dim, margin: 0 }}>
          Marque despesas como <b style={{ color: C.blue }}>Essencial</b> na aba Despesas para a meta ser calculada automaticamente.
        </p>
      </Card>
    )}
  </div>;
...
// SETTINGS
// ══════════════════════════════════════════
function SettingsPage({ user, onSignOut }: { user?: any; onSignOut?: () => void }) {
  const name = user?.user_metadata?.full_name || user?.email?.split("@")[0] || "Usuário";
  const initials = name.split(" ").map((p: string) => p[0]).join("").slice(0, 2).toUpperCase();
  return <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 520 }}>
    <Card>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
        <div style={{ width: 48, height: 48, borderRadius: 14, background: C.card2, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 18, fontWeight: 700, color: C.accent }}>{initials}</span></div>
        <div><p style={{ fontSize: 16, fontWeight: 600, color: C.text, margin: 0 }}>{name}</p><p style={{ fontSize: 12, color: C.dim, margin: 0 }}>{user?.email}</p></div>
      </div>
    ...
    <Btn onClick={onSignOut} style={{ justifyContent: "center", color: C.red }}>
...
function LoginPage() {
  ...
  const [error, setError] = useState<string | null>(null); const [info, setInfo] = useState<string | null>(null); const [loading, setLoading] = useState(false);
  ...
  async function handleSubmit() {
    ...
    if (mode === "login") {
      ...
      if (password.length < 6) { setLoading(false); setError("A senha precisa ter ao menos 6 caracteres."); return; }
  ...
  return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#080808" }}>
    <div className="fade-in" style={{ width: "100%", maxWidth: 380, padding: "40px 36px", background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: 22 }}>
      ...
      {mode !== "recover" && <>
        <button onClick={handleGoogle} style={{ width: "100%", padding: "11px", borderRadius: 10, border: "1px solid #1e1e1e", background: "#141414", color: C.sub, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {mode === "register" && <div><Label>Nome</Label><div style={inp("name")}><User size={13} color={C.dim} /><input placeholder="Seu nome" value={name} onChange={(e) => setName(e.target.value)} onFocus={() => setFocus("name")} onBlur={() => setFocus(null)} style={inpS} /></div></div>}
        <div><Label>Email</Label><div style={inp("email")}><Mail size={13} color={C.dim} /><input placeholder="seu@email.com" value={email} onChange={(e) => setEmail(e.target.value)} onFocus={() => setFocus("email")} onBlur={() => setFocus(null)} style={inpS} /></div></div>
        {mode !== "recover" && <div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><Label>Senha</Label>{mode === "login" && <span onClick={() => setMode("recover")} style={{ fontSize: 10, color: C.accent, cursor: "pointer" }}>Esqueceu?</span>}</div><div style={inp("pass")}><Lock size={13} color={C.dim} /><input type={showPass ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} onFocus={() => setFocus("pass")} onBlur={() => setFocus(null)} style={inpS} /><div onClick={() => setSP(!showPass)} style={{ cursor: "pointer" }}>{showPass ? <EyeOff size={13} color={C.dim} /> : <Eye size={13} color={C.dim} />}</div></div></div>}
      </div>
      {error && <p style={{ fontSize: 11, color: C.red, margin: "10px 0 0" }}>{error}</p>}
      {info && <p style={{ fontSize: 11, color: C.accent, margin: "10px 0 0" }}>{info}</p>}
      <button onClick={handleSubmit} disabled={loading} style={{ width: "100%", padding: "12px", borderRadius: 10, border: "none", background: C.accent, color: "#000", fontSize: 13, fontWeight: 600, cursor: loading ? "default" : "pointer", marginTop: 18, opacity: loading ? 0.7 : 1 }}>{loading ? "Aguarde..." : mode === "login" ? "Entrar" : mode === "register" ? "Criar conta" : "Enviar"}</button>
...
export default function Home() {
  const [session, setSession] = useState<Session | null | undefined>(undefined); // undefined = ainda checando
  ...
  const [reserve, setReserve] = useState<{ targetMonths: number; currentAmount: number }>({ targetMonths: 6, currentAmount: 0 });
  const [dataLoaded, setDataLoaded] = useState(false);
  const [expTrigger, setExpTrigger] = useState(0);
  const sw = col ? 64 : 230; const pc = expenses.filter(e => !e.paid).length;

  // checa sessão existente + escuta mudanças (login/logout)
  useEffect(() => {
  ...
  // ao logar, carrega (ou cria) a linha de dados do usuário
  useEffect(() => {
    if (!session?.user) { setDataLoaded(false); return; }
    (async () => {
      const { data: row } = await supabase.from("finance_data").select("*").eq("user_id", session.user.id).maybeSingle();
      if (row) {
        setIncome(row.income || []); setExpenses(row.expenses || []); setCards(row.cards || []);
        setSavings(row.savings || []); setReserve(row.reserve || { targetMonths: 6, currentAmount: 0 });
        setAuth(row.income?.length ? "app" : "onboarding");
      } else {
        await supabase.from("finance_data").insert({ user_id: session.user.id });
        setAuth("onboarding");
      }
      setDataLoaded(true);
    })();
  ...
  // salva na nuvem (com debounce) sempre que os dados mudam
  useEffect(() => {
    if (!session?.user || !dataLoaded) return;
    const t = setTimeout(() => {
      supabase.from("finance_data").update({ income, expenses, cards, savings, reserve, updated_at: new Date().toISOString() }).eq("user_id", session.user.id);
    }, 600);
    return () => clearTimeout(t);
  }, [income, expenses, cards, savings, reserve, session?.user?.id, dataLoaded]);

  if (session === undefined) return <div style={{ minHeight: "100vh", background: C.bg }} />;
  if (!session) return <LoginPage />;
  if (auth === "onboarding") {
    if (!dataLoaded) return <div style={{ minHeight: "100vh", background: C.bg }} />;
    return <OnboardingPage onComplete={(ii) => { setIncome(ii); setAuth("app"); }} />;
  }
  if (!dataLoaded) return <div style={{ minHeight: "100vh", background: C.bg }} />;

  const openExpenseModal = () => { setPage("expenses"); setExpTrigger(p => p + 1); };
  const goToIncome = () => setPage("income");
  const essentialMonthly = expenses.filter((e) => e.essential).reduce((s, e) => s + (e.value || 0), 0);
  ...
  return <div style={{ minHeight: "100vh", background: C.bg, color: C.text }}>
    ...
    <div style={{ marginLeft: sw, transition: "margin-left 0.3s cubic-bezier(.4,0,.2,1)" }}>
      ...
      <div style={{ padding: "22px 28px", maxWidth: 960 }}>
        ...
        {page === "reserve" && <ReservePage essentialMonthly={essentialMonthly} reserve={reserve} setReserve={setReserve} />}
        {page === "projections" && <ProjectionsPage income={income} expenses={expenses} />}
        {page === "settings" && <SettingsPage user={session.user} onSignOut={() => supabase.auth.signOut()} />}
      </div>
    </div>
  </div>;
}
