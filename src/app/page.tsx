...
=======
  ShoppingBag, Landmark, Banknote, Plus, Trash2, Check, Edit3, ChevronDown,
...
<<<<<<< HEAD
import { C, BRANDS, CATS, TYPE_LABELS, fmt, pct, uid, currentMonth, currentDay, daysLeft, daysInMonth, fmtDate } from "@/lib/constants";
...
// ══════════════════════════════════════════
=======
import { C, BRANDS, CATS, fmt, pct, uid, currentMonth, fmtDate } from "@/lib/constants";
import {
  Card, Progress, Tag, Empty, Btn, CurrencyInput, DateInput,
  Select, TextInput, Label, Modal, Checkbox,
} from "@/components/ui";
...
=======
    <div style={{
      width: collapsed ? 64 : 230, height: "100vh", background: C.bg,
      borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column",
      transition: "width 0.25s ease", overflow: "hidden",
      position: "fixed", left: 0, top: 0, zIndex: 100,
    }}>
      <div style={{ padding: collapsed ? "18px 16px" : "18px 22px", display: "flex", alignItems: "center", gap: 10, borderBottom: `1px solid ${C.border}`, minHeight: 60 }}>
        <div style={{ width: 32, height: 32, borderRadius: 9, background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <Zap size={15} color="#000" />
        </div>
        {!collapsed && (
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: C.text, lineHeight: 1 }}>Velloso</div>
            <div style={{ fontSize: 8, color: C.dim, letterSpacing: 1.5, textTransform: "uppercase" }}>finance</div>
          </div>
        )}
      </div>
        ...
        {NAV.map(it => {
          ...
          return (
            <div key={it.id} onClick={() => onNav(it.id)} style={{
              ...
              padding: collapsed ? "10px" : "10px 14px", borderRadius: 9,
              cursor: "pointer", background: ac ? C.card2 : "transparent",
              justifyContent: collapsed ? "center" : "flex-start",
            }}>
              <it.icon size={18} color={ac ? C.accent : C.dim} />
              {!collapsed && <span style={{ fontSize: 12.5, fontWeight: ac ? 600 : 400, color: ac ? C.text : C.sub }}>{it.label}</span>}
            </div>
          );
        })}
      </div>
      ...
      <div style={{ padding: 8, borderTop: `1px solid ${C.border}` }}>
        <div onClick={onToggle} style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 6, borderRadius: 6, cursor: "pointer" }}>
          {collapsed ? <ChevronRight size={15} color={C.dim} /> : <ChevronLeft size={15} color={C.dim} />}
        </div>
      </div>
    </div>
  );
...
function Header({ page, onNotif, notifOpen, pendingCount }: any) {
  ...
  return (
    <div style={{ height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 26px", borderBottom: `1px solid ${C.border}`, background: C.bg, position: "sticky", top: 0, zIndex: 50 }}>
      <h1 style={{ fontSize: 16, fontWeight: 600, color: C.text, margin: 0 }}>{titles[page]}</h1>
      <div onClick={onNotif} style={{
        width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center",
        background: notifOpen ? C.card2 : "transparent", border: `1px solid ${C.border}`, cursor: "pointer", position: "relative",
      }}>
        <Bell size={16} color={C.sub} />
        {pendingCount > 0 && <div style={{ position: "absolute", top: 7, right: 7, width: 7, height: 7, borderRadius: "50%", background: C.red, border: `2px solid ${C.bg}` }} />}
      </div>
    </div>
  );
}
...
function NotifPanel({ open, onClose, expenses }: any) {
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 56, right: 20, width: 300, zIndex: 201, background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, boxShadow: "0 12px 48px rgba(0,0,0,0.6)" }}>
        <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>Pendências</span>
          <X size={13} color={C.sub} style={{ cursor: "pointer" }} onClick={onClose} />
        </div>
        <div style={{ maxHeight: 280, overflowY: "auto" }}>
          {pending.length === 0 && <div style={{ padding: 20, textAlign: "center" }}><p style={{ fontSize: 11, color: C.dim }}>Tudo em dia!</p></div>}
          {pending.map((e: any, i: number) => (
            <div key={i} style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 9, color: C.dim, margin: 0 }}>{fmtDate(e.dueDay, e.dueMonth)}</p>
              </div>
              <span style={{ fontSize: 11, fontWeight: 600, color: C.red }}>{fmt(e.value)}</span>
            </div>
          ))}
        </div>
      </div>
...
const TT = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "8px 12px", boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
      <p style={{ fontSize: 9, color: C.dim, margin: "0 0 4px" }}>{label}</p>
      {payload.map((x: any, i: number) => (
        <p key={i} style={{ fontSize: 10, color: x.color, margin: "1px 0", fontWeight: 500 }}>{x.name}: {fmt(x.value)}</p>
      ))}
    </div>
  );
};
...
// DASHBOARD
// ═══════════════════════════════════════════════
function DashboardPage({ income, expenses, cards, onAddExpense, onNav }: any) {
  ...
  const rem = tI - tE - tC;
  const score = tI > 0 ? Math.max(0, Math.min(100, Math.round(((tI - tE - tC) / tI) * 100 + 30))) : 0;

  const catData = expenses.reduce((a: any[], e: any) => {
    const cat = CATS.find(x => x.id === e.category);
    if (cat) {
      const ex = a.find(x => x.name === cat.name);
      if (ex) ex.value += e.value;
      else a.push({ name: cat.name, value: e.value, color: cat.color });
    }
    return a;
  }, []).sort((a: any, b: any) => b.value - a.value);

  if (tI === 0 && tE === 0 && cards.length === 0) {
    return <Empty text="Comece cadastrando suas receitas e despesas" action={
      <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
        <Btn primary onClick={() => onNav("income")}><Plus size={12} /> Receita</Btn>
        <Btn onClick={() => onNav("expenses")}><Plus size={12} /> Despesa</Btn>
      </div>
    } />;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 12 }}>
        <Card style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <p style={{ fontSize: 11, color: C.sub, margin: "0 0 2px" }}>Saldo disponível</p>
          <p style={{ fontSize: 30, fontWeight: 700, color: rem >= 0 ? C.text : C.red, margin: "0 0 8px", letterSpacing: -1, fontVariantNumeric: "tabular-nums" }}>{fmt(rem)}</p>
          {score > 0 && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 6, background: score >= 50 ? `${C.green}12` : `${C.red}12`, alignSelf: "flex-start" }}>
              <Shield size={11} color={score >= 50 ? C.green : C.red} />
              <span style={{ fontSize: 10, fontWeight: 600, color: score >= 50 ? C.green : C.red }}>Saúde: {score}</span>
            </div>
          )}
        </Card>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <Btn primary onClick={onAddExpense} style={{ flex: 1, justifyContent: "center", borderRadius: 14 }}><Plus size={14} /> Novo Gasto</Btn>
          <Btn onClick={() => onNav("income")} style={{ flex: 1, justifyContent: "center", borderRadius: 14, background: C.card, border: `1px solid ${C.border}` }}><Plus size={14} /> Receita</Btn>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        {[
          ...
          <Card key={i} style={{ padding: 14 }}>
            ...
            <p style={{ fontSize: 17, fontWeight: 700, color: C.text, margin: 0, fontVariantNumeric: "tabular-nums" }}>{fmt(x.v)}</p>
          </Card>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {catData.length > 0 && (
          <Card>
            <p style={{ fontSize: 12, fontWeight: 500, color: C.text, margin: "0 0 10px" }}>Gastos por categoria</p>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 100, height: 100 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <RPie><Pie data={catData} dataKey="value" cx="50%" cy="50%" innerRadius={30} outerRadius={45} paddingAngle={2} strokeWidth={0}>
                    {catData.map((c: any, i: number) => <Cell key={i} fill={c.color} />)}
                  </Pie></RPie>
                </ResponsiveContainer>
              </div>
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
                {catData.slice(0, 5).map((c: any, i: number) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    ...
                    <span style={{ fontSize: 10, color: C.sub, flex: 1 }}>{c.name}</span>
                    <span style={{ fontSize: 10, color: C.text, fontWeight: 500, fontVariantNumeric: "tabular-nums" }}>{fmt(c.value)}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <Card>
          <p style={{ fontSize: 12, fontWeight: 500, color: C.text, margin: "0 0 10px" }}>Últimas despesas</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {expenses.length === 0 && <p style={{ fontSize: 11, color: C.dim }}>Nenhuma despesa</p>}
            {expenses.slice(-5).reverse().map((e: any) => {
              const cat = CATS.find(c => c.id === e.category);
              return (
                <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 8, background: C.card2 }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 11, color: C.text, margin: 0 }}>{e.name}</p>
                    <p style={{ fontSize: 9, color: C.dim, margin: 0 }}>{fmtDate(e.dueDay, e.dueMonth)}</p>
                  </div>
                  <span style={{ fontSize: 11, fontWeight: 600, color: C.red, fontVariantNumeric: "tabular-nums" }}>{fmt(e.value)}</span>
                </div>
              );
            })}
          </div>
      ...
      {cards.length > 0 && (
        <Card>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            <p style={{ fontSize: 12, fontWeight: 500, color: C.text, margin: 0 }}>Cartões</p>
            <span onClick={() => onNav("cards")} style={{ fontSize: 10, color: C.accent, cursor: "pointer" }}>Ver todos →</span>
          </div>
          <div style={{ display: "flex", gap: 8, overflowX: "auto" }}>
            {cards.map((c: any) => {
              const b = BRANDS[c.brand] || BRANDS.outro;
              return (
                <div key={c.id} style={{ minWidth: 160, padding: 14, borderRadius: 12, background: b.bg, color: (b as any).textColor || "#fff", flexShrink: 0 }}>
                  <p style={{ fontSize: 12, fontWeight: 600, margin: "0 0 2px" }}>{b.name}</p>
                  <p style={{ fontSize: 16, fontWeight: 700, margin: "4px 0", fontVariantNumeric: "tabular-nums" }}>{fmt(c.used)}</p>
                  <p style={{ fontSize: 9, opacity: 0.7, margin: 0 }}>de {fmt(c.limit)}</p>
                </div>
              );
            })}
          </div>
...
function IncomePage({ income, setIncome }: any) {
  ...
  const tL: any = { salario: "Salário", va: "Vale Adiantamento", vt: "Vale Transporte", extra: "Renda Extra", comissao: "Comissão", freelance: "Freelance" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <p style={{ fontSize: 10, color: C.sub }}>Total do mês</p>
          <p style={{ fontSize: 24, fontWeight: 700, color: C.green, fontVariantNumeric: "tabular-nums" }}>{fmt(total)}</p>
        </div>
      ...
      {income.map((i: any) => (
        <Card key={i.id} style={{ padding: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: `${C.green}10` }}>
              <Banknote size={16} color={C.green} />
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 13, fontWeight: 500, color: C.text, margin: 0 }}>{i.name}</p>
              <p style={{ fontSize: 10, color: C.dim, margin: 0 }}>{fmtDate(i.day, i.month)} · {tL[i.type] || i.type}</p>
            </div>
            <span style={{ fontSize: 16, fontWeight: 600, color: C.green, fontVariantNumeric: "tabular-nums", marginRight: 8 }}>+{fmt(i.value)}</span>
            <Edit3 size={13} color={C.dim} style={{ cursor: "pointer", marginRight: 4 }} onClick={() => openEdit(i)} />
            <Trash2 size={13} color={C.dim} style={{ cursor: "pointer" }} onClick={() => setIncome(income.filter((x: any) => x.id !== i.id))} />
          </div>
        </Card>
      ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? "Editar Receita" : "Nova Receita"}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div><Label>Valor</Label><CurrencyInput value={value} onChange={setValue} /></div>
          <div><Label>Data (dia/mês)</Label><DateInput day={day} month={month} onDayChange={setDay} onMonthChange={setMonth} /></div>
          <div><Label>Tipo</Label><Select value={type} onChange={setType} options={Object.entries(tL).map(([v, l]: any) => ({ value: v, label: l }))} /></div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 16 }}>
          <div><p style={{ fontSize: 9, color: C.sub }}>Total</p><p style={{ fontSize: 18, fontWeight: 700, color: C.red, fontVariantNumeric: "tabular-nums" }}>{fmt(total)}</p></div>
          <div><p style={{ fontSize: 9, color: C.sub }}>Pendente</p><p style={{ fontSize: 18, fontWeight: 700, color: C.yellow, fontVariantNumeric: "tabular-nums" }}>{fmt(pending)}</p></div>
        </div>
      ...
      <div style={{ display: "flex", gap: 5 }}>
        {[{ id: "all", l: "Todas" }, { id: "pending", l: "Pendentes" }, { id: "paid", l: "Pagas" }, { id: "essential", l: "Essenciais" }].map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)} style={{
            ...
            border: `1px solid ${filter === f.id ? C.accent + "60" : C.border}`,
            background: filter === f.id ? `${C.accent}10` : "transparent",
            color: filter === f.id ? C.accent : C.dim, fontSize: 10, fontWeight: 500, cursor: "pointer",
          }}>{f.l}</button>
        ))}
      </div>
      ...
      {fl.map((e: any) => {
        ...
        return (
          <Card key={e.id} style={{ padding: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", background: `${cat?.color || C.dim}10` }}>
                <Receipt size={14} color={cat?.color || C.dim} />
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 12, fontWeight: 500, color: C.text, margin: 0 }}>{e.name}</p>
              ...
              <div style={{ textAlign: "right", marginRight: 4 }}>
                <p style={{ fontSize: 13, fontWeight: 600, color: C.text, margin: 0, fontVariantNumeric: "tabular-nums" }}>{fmt(e.value)}</p>
              ...
              <Edit3 size={12} color={C.dim} style={{ cursor: "pointer" }} onClick={() => openEdit(e)} />
              <Trash2 size={12} color={C.dim} style={{ cursor: "pointer" }} onClick={() => setExpenses(expenses.filter((x: any) => x.id !== e.id))} />
            </div>
          </Card>
        );
      })}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? "Editar Despesa" : "Nova Despesa"}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Checkbox checked={essential} onChange={() => setEssential(!essential)} label="Essencial" />
          <Btn primary onClick={save} style={{ width: "100%", justifyContent: "center", marginTop: 4 }}>{editId ? "Salvar" : "Adicionar"}</Btn>
        </div>
      </Modal>
    </div>
...
// ═══════════════════════════════════════════════
// CARDS — clickable with purchases
// ═══════════════════════════════════════════════
function CardsPage({ cards, setCards }: any) {
  const [modal, setModal] = useState(false);
  const [purchaseModal, setPurchaseModal] = useState<number | null>(null);
  const [brand, setBrand] = useState("nubank");
  const [limit, setLimit] = useState(0);
  const [pName, setPName] = useState("");
  const [pValue, setPValue] = useState(0);
  const [pDay, setPDay] = useState("");
  const [pMonth, setPMonth] = useState(String(currentMonth));
  const [openCard, setOpenCard] = useState<number | null>(null);

  const addCard = () => {
    if (!limit) return;
    setCards([...cards, { id: uid(), brand, limit, used: 0, purchases: [] }]);
    setLimit(0); setModal(false);
  };
  ...
  const addPurchase = (cardId: number) => {
    ...
    setCards(cards.map((c: any) => {
      if (c.id === cardId) {
        const np = [...c.purchases, { id: uid(), name: pName, value: pValue, day: pDay, month: pMonth }];
        return { ...c, purchases: np, used: np.reduce((s: number, p: any) => s + p.value, 0) };
      }
      return c;
    }));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <Btn primary onClick={() => setModal(true)}><Plus size={13} /> Novo Cartão</Btn>
      </div>
      ...
      {cards.map((c: any) => {
        ...
        const bc = u > 70 ? C.red : u > 40 ? C.yellow : C.green;
        const isOpen = openCard === c.id;

        return (
          <div key={c.id}>
            <div onClick={() => setOpenCard(isOpen ? null : c.id)} style={{
              padding: 18, borderRadius: isOpen ? "14px 14px 0 0" : 14,
              background: b.bg, color: (b as any).textColor || "#fff", cursor: "pointer", position: "relative",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <p style={{ fontSize: 16, fontWeight: 700, margin: "0 0 2px" }}>{b.name}</p>
                  <p style={{ fontSize: 22, fontWeight: 700, margin: "6px 0", fontVariantNumeric: "tabular-nums" }}>{fmt(c.used)}</p>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <div onClick={e => { e.stopPropagation(); setPurchaseModal(c.id); }} style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    <Plus size={13} />
                  </div>
                  <div onClick={e => { e.stopPropagation(); setCards(cards.filter((x: any) => x.id !== c.id)); }} style={{ width: 28, height: 28, borderRadius: 7, background: "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                    <Trash2 size={13} />
                  </div>
                </div>
              </div>
              <div style={{ marginTop: 8 }}><Progress value={c.used} max={c.limit} color={bc} h={4} /></div>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                <span style={{ fontSize: 10, opacity: 0.6 }}>Limite: {fmt(c.limit)}</span>
                <span style={{ fontSize: 10, opacity: 0.6 }}>Disponível: {fmt(c.limit - c.used)}</span>
              </div>
              <div style={{ textAlign: "center", marginTop: 6 }}>
                <ChevronDown size={14} style={{ opacity: 0.4, transform: isOpen ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s" }} />
              </div>
            </div>

            {isOpen && (
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderTop: "none", borderRadius: "0 0 14px 14px", padding: 14 }}>
                <p style={{ fontSize: 11, fontWeight: 500, color: C.text, margin: "0 0 8px" }}>Compras na fatura</p>
                {c.purchases.length === 0 && <p style={{ fontSize: 10, color: C.dim }}>Nenhuma compra lançada</p>}
                {c.purchases.map((p: any) => (
                  <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 7, background: C.card2, marginBottom: 4 }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 11, fontWeight: 500, color: C.text, margin: 0 }}>{p.name}</p>
                      <p style={{ fontSize: 9, color: C.dim, margin: 0 }}>{fmtDate(p.day, p.month)}</p>
                    </div>
                    <span style={{ fontSize: 12, fontWeight: 600, color: C.text, fontVariantNumeric: "tabular-nums" }}>{fmt(p.value)}</span>
                    <Trash2 size={11} color={C.dim} style={{ cursor: "pointer" }} onClick={() => {
                      setCards(cards.map((card: any) => {
                        if (card.id === c.id) {
                          const np = card.purchases.filter((x: any) => x.id !== p.id);
                          return { ...card, purchases: np, used: np.reduce((s: number, x: any) => s + x.value, 0) };
                        }
                        return card;
                      }));
                    }} />
                ...
                <div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 8, background: `${bc}10`, display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 11, color: C.sub }}>Total fatura</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: bc }}>{fmt(c.used)}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Novo Cartão">
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div><Label>Limite</Label><CurrencyInput value={limit} onChange={setLimit} /></div>
      ...
      <Modal open={!!purchaseModal} onClose={() => setPurchaseModal(null)} title="Nova Compra">
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div><Label>Valor</Label><CurrencyInput value={pValue} onChange={setPValue} /></div>
...
function SavingsPage({ savings, setSavings }: any) {
  ...
  const icons = [Shield, Star, Car, PiggyBank, Home];
  const colors = [C.green, C.yellow, C.blue, C.purple, C.accent];

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div><p style={{ fontSize: 10, color: C.sub }}>Total guardado</p><p style={{ fontSize: 22, fontWeight: 700, color: C.green, fontVariantNumeric: "tabular-nums" }}>{fmt(total)}</p></div>
        <Btn primary onClick={() => setModal(true)}><Plus size={13} /> Nova Meta</Btn>
      </div>

      {savings.length === 0 && <Empty text="Nenhuma meta cadastrada" />}
      {savings.map((s: any, i: number) => {
        const Icon = icons[i % icons.length];
        const color = colors[i % colors.length];
        const p = s.goal > 0 ? (s.current / s.goal) * 100 : 0;
        return (
          <Card key={s.id}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div style={{ width: 34, height: 34, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", background: `${color}10` }}>
                <Icon size={15} color={color} />
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: 13, fontWeight: 500, color: C.text, margin: 0 }}>{s.label}</p>
                <p style={{ fontSize: 10, color: C.dim, margin: 0 }}>{fmt(s.current)} de {fmt(s.goal)}</p>
              </div>
              <span style={{ fontSize: 16, fontWeight: 700, color }}>{pct(p)}</span>
              <Trash2 size={12} color={C.dim} style={{ cursor: "pointer" }} onClick={() => setSavings(savings.filter((x: any) => x.id !== s.id))} />
            </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Nova Meta">
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div><Label>Objetivo</Label><CurrencyInput value={goal} onChange={setGoal} /></div>
          <div><Label>Já guardou</Label><CurrencyInput value={current} onChange={setCurrent} /></div>
...
// ═══════════════════════════════════════════════
// PROJECTIONS — detailed per expense per month
// ═══════════════════════════════════════════════
function ProjectionsPage({ income, expenses }: any) {
  ...
  useEffect(() => {
    ...
    setExtraIncome(months.map(() => bI));
    setGen(false);
  }, [expenses.length, bI]);

  const chartData = months.map((m, mi) => {
    const totalExp = projExp.reduce((s: number, e: any) => s + (e.months?.[mi] || 0), 0);
    return { month: m, income: extraIncome[mi] || 0, expenses: totalExp, saved: Math.max(0, (extraIncome[mi] || 0) - totalExp) };
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <Card>
        <p style={{ fontSize: 12, fontWeight: 500, color: C.text, margin: "0 0 14px" }}>Personalize cada gasto por mês</p>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", fontSize: 10 }}>
            <thead>
              <tr>
                <th style={{ textAlign: "left", padding: "6px 8px", color: C.dim, fontWeight: 600, position: "sticky", left: 0, background: C.card, minWidth: 100 }}>Gasto</th>
                {months.map(m => <th key={m} style={{ padding: "6px 4px", color: C.dim, fontWeight: 600, textAlign: "center", minWidth: 90 }}>{m}</th>)}
              </tr>
            </thead>
            <tbody>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                <td style={{ padding: "6px 8px", color: C.green, fontWeight: 600, position: "sticky", left: 0, background: C.card }}>Receita</td>
                {months.map((_, mi) => <td key={mi} style={{ padding: 4 }}>
                  <CurrencyInput value={extraIncome[mi] || 0} onChange={v => { const ni = [...extraIncome]; ni[mi] = v; setExtraIncome(ni); }} />
                </td>)}
              </tr>
              {projExp.map((e: any, ei: number) => (
                <tr key={e.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                  <td style={{ padding: "6px 8px", color: C.text, fontWeight: 500, position: "sticky", left: 0, background: C.card }}>{e.name}</td>
                  {months.map((_, mi) => <td key={mi} style={{ padding: 4 }}>
                    <CurrencyInput value={e.months?.[mi] || 0} onChange={v => {
                      const np = [...projExp];
                      np[ei] = { ...np[ei], months: [...(np[ei].months || [])] };
                      np[ei].months[mi] = v;
                      setProjExp(np);
                    }} />
                  </td>)}
              ...
              <tr>
                <td style={{ padding: "6px 8px", color: C.accent, fontWeight: 600, position: "sticky", left: 0, background: C.card }}>Sobra</td>
                {chartData.map((d, i) => <td key={i} style={{ padding: "6px 4px", textAlign: "center", fontWeight: 600, color: d.saved >= 0 ? C.green : C.red, fontSize: 11 }}>{fmt(d.saved)}</td>)}
              </tr>
            </tbody>
          </table>
        </div>
        <Btn primary onClick={() => setGen(true)} style={{ width: "100%", justifyContent: "center", marginTop: 14 }}>Gerar Projeção</Btn>
      </Card>

      {gen && (
        <Card>
          <p style={{ fontSize: 12, fontWeight: 500, color: C.text, margin: "0 0 14px" }}>Projeção Visual</p>
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <YAxis tick={{ fill: C.dim, fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}k`} />
                <Tooltip content={<TT />} />
                <Bar dataKey="income" fill={C.green} radius={[4, 4, 0, 0]} name="Receitas" />
                <Bar dataKey="expenses" fill={C.red} radius={[4, 4, 0, 0]} name="Despesas" />
                <Bar dataKey="saved" fill={C.purple} radius={[4, 4, 0, 0]} name="Sobra" />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 500 }}>
      <Card>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: C.card2, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: C.accent }}>LV</span>
          </div>
          <div>
            <p style={{ fontSize: 14, fontWeight: 600, color: C.text, margin: 0 }}>Lucas Velloso</p>
            <p style={{ fontSize: 11, color: C.dim, margin: 0 }}>lucas@email.com</p>
          </div>
        </div>
        {[{ l: "Perfil", v: "CLT" }, { l: "Meta", v: "15%" }, { l: "Moeda", v: "BRL" }].map((f, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
            <span style={{ fontSize: 11, color: C.sub }}>{f.l}</span>
            <span style={{ fontSize: 11, color: C.text, fontWeight: 500 }}>{f.v}</span>
          </div>
        ))}
      </Card>
    </div>
...
function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  const inp = (f: string) => ({
    ...
    background: focus === f ? "#1a1a1a" : "#141414",
    border: `1px solid ${focus === f ? "#333" : "#1e1e1e"}`,
    transition: "all 0.2s",
  });
  const inpS: React.CSSProperties = { background: "transparent", border: "none", color: C.text, fontSize: 12, outline: "none", width: "100%" };

  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#080808" }}>
      <div style={{ width: "100%", maxWidth: 360, padding: "36px 32px", background: "#0f0f0f", border: "1px solid #1c1c1c", borderRadius: 18 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: C.accent, display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
            <Zap size={18} color="#000" />
          </div>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.text }}>Velloso</div>
          <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, textTransform: "uppercase", marginTop: 2 }}>Finance</div>
        </div>

        {mode !== "recover" && (
          <>
            <button style={{ width: "100%", padding: "10px", borderRadius: 9, border: "1px solid #1e1e1e", background: "#141414", color: C.sub, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "16px 0" }}>
              ...
              <span style={{ fontSize: 9, color: C.dim }}>ou</span>
              <div style={{ flex: 1, height: 1, background: "#1e1e1e" }} />
            </div>
          </>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {mode === "register" && <div><Label>Nome</Label><div style={inp("name")}><User size={12} color={C.dim} /><input placeholder="Seu nome" onFocus={() => setFocus("name")} onBlur={() => setFocus(null)} style={inpS} /></div></div>}
          <div><Label>Email</Label><div style={inp("email")}><Mail size={12} color={C.dim} /><input placeholder="seu@email.com" onFocus={() => setFocus("email")} onBlur={() => setFocus(null)} style={inpS} /></div></div>
          {mode !== "recover" && <div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
              <Label>Senha</Label>
              {mode === "login" && <span onClick={() => setMode("recover")} style={{ fontSize: 9, color: C.accent, cursor: "pointer" }}>Esqueceu?</span>}
            </div>
            <div style={inp("pass")}><Lock size={12} color={C.dim} /><input type={showPass ? "text" : "password"} placeholder="••••••••" onFocus={() => setFocus("pass")} onBlur={() => setFocus(null)} style={inpS} /><div onClick={() => setSP(!showPass)} style={{ cursor: "pointer" }}>{showPass ? <EyeOff size={12} color={C.dim} /> : <Eye size={12} color={C.dim} />}</div></div>
          </div>}
        </div>

        <button onClick={onLogin} style={{ width: "100%", padding: "11px", borderRadius: 9, border: "none", background: C.accent, color: "#000", fontSize: 12, fontWeight: 600, cursor: "pointer", marginTop: 16 }}>
          {mode === "login" ? "Entrar" : mode === "register" ? "Criar conta" : "Enviar"}
        </button>
        <div style={{ textAlign: "center", marginTop: 14 }}>
          {mode === "login"
            ? <span style={{ fontSize: 11, color: C.dim }}>Sem conta? <span onClick={() => setMode("register")} style={{ color: C.sub, cursor: "pointer", fontWeight: 500 }}>Criar</span></span>
            : <span style={{ fontSize: 11, color: C.dim }}>Tem conta? <span onClick={() => setMode("login")} style={{ color: C.sub, cursor: "pointer", fontWeight: 500 }}>Entrar</span></span>
          }
        </div>
      </div>
    </div>
...
function OnboardingPage({ onComplete }: { onComplete: (income: any[]) => void }) {
  ...
  const [savePct, setSP] = useState(15);
  const [hover, setH] = useState<string | null>(null);
  ...
  const profiles = [
    { id: "clt", label: "CLT", icon: Landmark },
    { id: "autonomo", label: "Autônomo", icon: Zap },
    { id: "freelancer", label: "Freelancer", icon: Smartphone },
  ...
  const steps = [
    {
      ...
      content: (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {profiles.map(p => (
            <div key={p.id} onClick={() => setProfile(p.id)} onMouseEnter={() => setH(p.id)} onMouseLeave={() => setH(null)} style={{
              padding: 18, borderRadius: 12, cursor: "pointer", textAlign: "center",
              background: profile === p.id ? "#1e1e1e" : hover === p.id ? "#181818" : C.card,
              border: `2px solid ${profile === p.id ? C.accent : hover === p.id ? "#333" : C.border}`, transition: "all 0.2s",
            }}>
              <p.icon size={22} color={profile === p.id ? C.accent : C.dim} style={{ marginBottom: 4 }} />
              <div style={{ fontSize: 12, fontWeight: 600, color: profile === p.id ? C.text : C.sub }}>{p.label}</div>
            </div>
          ))}
        </div>
      ),
    },
    {
      ...
      content: (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div><Label>Salário</Label><CurrencyInput value={sal} onChange={setSal} /></div>
          <div><Label>Vale Adiantamento</Label><CurrencyInput value={va} onChange={setVa} /></div>
          <div><Label>Vale Transporte</Label><CurrencyInput value={vt} onChange={setVt} /></div>
          <div><Label>Renda Extra</Label><CurrencyInput value={ex} onChange={setEx} /></div>
        </div>
      ),
    },
    {
      ...
      content: (
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            {[5, 10, 15, 20].map(p => (
              <div key={p} onClick={() => setSP(p)} onMouseEnter={() => setH(`p${p}`)} onMouseLeave={() => setH(null)} style={{
                flex: 1, padding: 14, borderRadius: 10, textAlign: "center", cursor: "pointer",
                background: savePct === p ? "#1e1e1e" : hover === `p${p}` ? "#181818" : C.card,
                border: `2px solid ${savePct === p ? C.accent : hover === `p${p}` ? "#333" : C.border}`, transition: "all 0.2s",
              }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: savePct === p ? C.accent : C.text }}>{p}%</div>
              </div>
            ))}
          </div>
          <div style={{ padding: 12, borderRadius: 9, background: `${C.green}08`, textAlign: "center" }}>
            <span style={{ fontSize: 11, color: C.green }}>≈ {fmt((sal + va + vt + ex) * savePct / 100)}/mês</span>
          </div>
        </div>
      ),
    },
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg }}>
      <div style={{ width: "100%", maxWidth: 440, padding: 32 }}>
        <div style={{ display: "flex", gap: 5, marginBottom: 30 }}>
          {steps.map((_, i) => <div key={i} style={{ flex: 1, height: 3, borderRadius: 3, background: i <= step ? C.accent : "#1e1e1e" }} />)}
        </div>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: C.text, margin: "0 0 18px", textAlign: "center" }}>{steps[step].title}</h2>
        {steps[step].content}
        <div style={{ display: "flex", gap: 8, marginTop: 22 }}>
          {step > 0 && <Btn onClick={() => setStep(step - 1)}>Voltar</Btn>}
          <Btn primary onClick={() => step < steps.length - 1 ? setStep(step + 1) : finish()} style={{ flex: 1, justifyContent: "center" }}>
...
export default function Home() {
  ...
  const sw = col ? 64 : 230;
  const pc = expenses.filter(e => !e.paid).length;
  ...
  const openExpenseModal = () => { setPage("expenses"); setExpTrigger(prev => prev + 1); };

  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text }}>
      <Sidebar active={page} onNav={setPage} collapsed={col} onToggle={() => setCol(!col)} />
      <div style={{ marginLeft: sw, transition: "margin-left 0.25s ease" }}>
        ...
        <div style={{ padding: "20px 24px", maxWidth: 920 }}>
          {page === "dashboard" && <DashboardPage income={income} expenses={expenses} cards={cards} onAddExpense={openExpenseModal} onNav={setPage} />}
